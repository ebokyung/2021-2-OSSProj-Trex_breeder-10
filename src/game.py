                    ...
                    if (len(m_list)==0):
                        ...
                        if (m.x>=pking.rect.left)and(m.x<=pking.rect.right)and(m.y>pking.rect.top)and(m.y<pking.rect.bottom):
                            ...
                            if pking.hp <= 0:
                                pking.kill()
                                isPkingAlive=False
                                # 남현 - 211031 pking이 죽었을 때 다음스테이지로 넘어가게 함(gameplay_hard2호출)
                                # 현재 스피드와 점수를 파라미터로 넘겨줌
                                print("다음 스테이지 진입")
                                gameplay_hard2(gamespeed, playerDino.score)
...
#남현 - 211031 스테이지 추가를 위한 gameplay_hard() 함수 복사
def gameplay_hard2(sp, sc):
    ...
    #남현 - 211031 이전 스테이지에서의 스피드를 받아 옴
    gamespeed = sp
