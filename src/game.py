...
def gameplay_hard(cur_stage=1, cur_life=15, cur_speed=4, cur_score=0):
    ...
    max_life = 15
    life = cur_life  
    global gamespeed #게임을 실행하면 실제 움직이는 initial gamespeed는 4(4~13)이고, 사용자에게는 가장 낮은 speed인 1(1~10)로 인식하는 값임
    gamespeed = cur_speed
    def gamespeed_down():
        global gamespeed
        if gamespeed > 4:
                    ...
                    if total_time - elapsed_time <= 0:
                        ...
                        if (isBossKilled == False) and (stage!=0):
                            ...
                            if (stage == 1):
                                ...
                                pygame.time.wait(500)
                                gameplay_hard( stage-2, life, gamespeed, playerDino.score )
                            elif (stage == 0):
                                pygame.time.wait(500)
                                gameplay_hard( stage+3, life, gamespeed, playerDino.score )
                            elif (stage == 3):
                                print("모든 스테이지 클리어")
                                pygame.time.wait(500)
